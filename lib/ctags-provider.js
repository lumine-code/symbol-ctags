const { BufferedProcess, CompositeDisposable, FileState, watchFile, Point } = require("lumine");
...
class CtagsProvider {
  ...
  canProvideSymbols(meta) {
    ...
    if (editor.getFileState() !== FileState.UNMODIFIED) score -= 0.1;
  ...
  getSymbols(meta) {
    ...
    if (lumine.config.get("symbol-ctags.useEditorGrammarAsCtagsLanguage")) {
  ...
  async getSymbolsInProject() {
    ...
    return tags.map((tag) => this.interpretTag(tag));
  }
