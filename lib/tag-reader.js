const { Point, Task } = require("lumine");
...
module.exports = {
  async find(editor) {
    ...
    if (!symbols.length) {
      ...
      if (hasRubyScope) {
        nonWordCharacters = lumine.config.get("language.nonWordCharacters", { scope });
  ...
  getAllTags() {
    ...
    return new Promise((resolve) => {
      let task = Task.once(handlerPath, lumine.project.getPaths(), () => {
        resolve(projectTags);
      });
